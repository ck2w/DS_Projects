...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle:
        comb = list(zip(data, label))
        random.shuffle(comb)
        b_data, b_label = zip(*comb)
        for i in range(batches):
            batched_data.append(np.array(b_data[i * batch_size: (i + 1) * batch_size]))
            batched_label.append(np.array(b_label[i * batch_size: (i + 1) * batch_size]))
    else:
        for i in range(batches):
            batched_data.append(np.array(data[i*batch_size : (i+1)*batch_size]))
            batched_label.append(np.array(label[i * batch_size: (i + 1) * batch_size]))
