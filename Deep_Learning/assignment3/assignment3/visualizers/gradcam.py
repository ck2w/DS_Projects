...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        loss = torch.sum(scores.gather(1, y_tensor.view(-1, 1)).squeeze())
        loss.backward()
        ...
        #
        # gc_pred = gc_model.forward(X_tensor)
        #
        # y_reshape = y_tensor.view(-1, 1)  # reshape y_tensor from (N,) to (N,1)
        # loss_tmp = torch.gather(gc_pred, 1, y_reshape)  # collecting the right label based on the dims of pred
        # loss = torch.squeeze(loss_tmp)  # squeeze it into low dims
        #
        # for num in range(N):
        #     loss[num].backward(retain_graph=True)  # update grad using backward function
        #
        # g_backprop = X_tensor.grad.permute(0, 2, 3, 1)  # get the initial x and switch from [N,3,H,W] to [N,H,W,3]
        # gd_backpp_array = g_backprop.numpy()  # turn a tensor to a numpy array
        #
        # return gd_backpp_array

        # predictions = gc_model.forward(X_tensor)
        # labels = y_tensor.view(-1, 1)
        # compare = predictions.gather(1, labels).squeeze()
        # upstream_grad = torch.ones_like(compare)
        # compare.backward(upstream_grad, retain_graph=True)
        # backprop = X_tensor.grad
        # backprop = backprop.permute(0, 2, 3, 1).detach().numpy()
        # return backprop
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        loss = torch.sum(scores.gather(1, y_tensor.view(-1, 1)).squeeze())
        loss.backward()
        grads = torch.mean(self.gradient_value.data, (2, 3), keepdim=True)
        cam = torch.nn.functional.relu(torch.sum(self.activation_value.data * grads, 1)).numpy()
