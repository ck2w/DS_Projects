...
class Helper():
  ...
  def mse_key(self, history=None, model=None):

    key_name = None
    
    if model is not None:
      key_name = "mse" if "mse" in model.metrics_names else "mse"
    else:
      key_name = "mse" if "mse" in history.history.keys() else "loss"
  ...
  def plotTrain(self, history, model_name="???"):
    ...
    mse_string = self.mse_key(history=history)
    ...
    # Plot accuracy
    axs[1].plot(history.history[ mse_string ])
    axs[1].plot(history.history['val_' + mse_string ])
    axs[1].set_title(model_name + " " +'model mse')
